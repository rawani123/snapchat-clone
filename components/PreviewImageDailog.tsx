...
export function DialogDemo({selectedFile,close,imageChange,setFlag}:{selectedFile:string,close:()=> void,imageChange:any,setFlag:any}) {
  return (
    <Dialog open={!!selectedFile}>
      ...
      <DialogContent onInteractOutside={close} className="sm:max-w-[425px] bg-white border max-w-xl  flex flex-col">
        ...
        <DialogFooter className="mx-auto flex items-center">
            <DialogClose asChild>
                <Button className="rounded-full" variant={'destructive'} size={'sm'} onClick={close}>Cancel</Button>
            </DialogClose>
